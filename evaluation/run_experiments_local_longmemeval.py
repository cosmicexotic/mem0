...
def main():
    ...
    parser.add_argument('--dataset_name', type=str, default="longmemeval_example",
                        help='Dataset name')
    ...
    data_path = f'evaluation/dataset/{args.dataset_name}.json'
    ...
    if args.technique_type == "mem0":
        # if args.method == "add":
            ...
            memory_manager.process_all_conversations(dataset="longmemeval")
        # elif args.method == "search": 
            output_file_path = os.path.join(
                args.output_folder,                 
                f"{args.dataset_name}_mem0_results_top_{args.top_k}_filter_{args.filter_memories}_graph_{args.is_graph}.json"
            )
            memory_searcher = MemorySearch(
                memory_manager.mem0_client,
                output_file_path,
                args.top_k,
                args.filter_memories,
                args.is_graph
            )
            memory_searcher.process_data_file_longmemeval(data_path)
