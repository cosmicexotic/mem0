...
from metrics.llm_judge import evaluate_llm_judge, evaluate_llm_judge_longmemeval
...
def process_item_longmemeval(item_data):
    k, v = item_data
    local_results = defaultdict(list)
    ...
    for item in v:
        ...
        metrics = calculate_metrics(pred_answer, gt_answer)
        bleu_scores = calculate_bleu_scores(pred_answer, gt_answer)
        llm_score = evaluate_llm_judge_longmemeval(question, gt_answer, pred_answer, category)
...
def main():
    ...
    parser.add_argument(
        "--dataset", type=str, default="locomo", help="Dataset to evaluate"
    )
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        if args.dataset == "locomo":
            ...
            futures = [executor.submit(process_item_longmemeval, item_data) for item_data in data.items()]
