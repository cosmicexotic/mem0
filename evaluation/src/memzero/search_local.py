...
class MemorySearch:
    ...
    def __init__(self, memory_client, output_path='results.json', top_k=10, filter_memories=False, is_graph=False):
        self.mem0_client = memory_client
        self.top_k = top_k
    ...
    def search_memory(self, user_id, query, max_retries=3, retry_delay=1):
        ...
        if not self.is_graph:
            print(f"Retrieving memories {memories} {type(memories)}")
            semantic_memories = [
                {
    ...
    def process_questions_longmemeval(self, question_item, speaker_a_user_id, speaker_b_user_id):
        question = question_item['question']
        answer = question_item['answer']
        category = question_item['question_type']
        evidence = question_item['answer_session_ids']
        adversarial_answer = ""
    ...
    def process_data_file_longmemeval(self, file_path):
        with open(file_path, 'r') as f:
            data = json.load(f)
        ...
        for idx, item in tqdm(enumerate(data), total=len(data), desc="Processing conversations"):
            ...
            question_item = item
            result = self.process_questions_longmemeval(
                question_item,
                speaker_a_user_id,
                speaker_b_user_id
            )
